...
![](https://github.com/Cloudox/iOS-Human-Interface-Guidelines/blob/master/UI%20Elements/Content%20Views/28.jpeg)

`API NOTE`  
查看[UIDatePicker](https://developer.apple.com/library/ios/documentation/UIKit/Reference/UIDatePicker_Class/index.html#//apple_ref/occ/cl/UIDatePicker)来学习如何在你的代码中定义一个日期选取器。

一个日期选取器：
* 显示最多四个独立的滚轮，每一个都显示一个类的值，例如月份或小时
* 使用使用深色文本在视图的中间显示当前的值
* 不能调整大小（日期选取器的大小和iPhone键盘的大小一样）
* 有四种模式，每一种都显示包含一系列不同值的滚轮：
 * `日期和时间。`日期和事件模式（默认的模式）显示日历日期、小时和分钟的滚轮，以及一个可选的AM/PM滚轮。
 * `时间。`时间模式显示小时和分钟的滚轮，以及一个可选的AM/PM滚轮。
 * `日期。`日期模式显示月、日和年的滚轮。
 * `定时器。`定时器模式显示小时和分钟的滚轮。你可以设定倒计时的完整周期，最大23小时59分钟。

使用日期选取器让用户选择——替代输入——一个多部分组合成的日期或事件，比如日、月和年。

`尽可能地将日期选取器与内容内联。`用户不用导航到另一个视图去使用日期选取器是最好的。在水平常规环境下，日期选取器可以通过弹层显示或者与内容内联。

`如果对你的app有意义的话，更改分钟滚轮的间隔。`默认情况下分钟滚轮显示60个值（从0到59）。如果你需要显示一个粗粒度的选项，你可以设置一个分钟滚轮来显示一个大一些的分钟间隔，只要间隔可以均分60分钟。比如说，你可能想要显示四分之一小时的间隔0、15/30和45。
...
![](https://github.com/Cloudox/iOS-Human-Interface-Guidelines/blob/master/UI%20Elements/Controls/5.png)

`API NOTE`  
查看[UIPageControl](https://developer.apple.com/library/ios/documentation/UIKit/Reference/UIPageControl_Class/index.html#//apple_ref/occ/cl/UIPageControl)学习更多关于在你的代码中定义一个页面控制器的内容。

一个页面控制器：
* 为每个app中打开的视图显示一个点指示器（从左到右，点代表了视图打开的顺序）
* 默认情况下，使用不透明点来表示当前看到的视图，半透明的点表示所有打开的视图
* 不允许用户不连续地查看视图
* 不要在打开了很多视图的时候把点收缩到一起（如果你尝试显示超过视图大小的点数量，点会剪除一部分）
* 不要默认允许在视图间导航；你必须实现视图到视图间的导航并且适当地更新页面控制器的状态

当告诉用户有多少视图打开了比帮助他们选择一个特定的视图更重要时，使用页面控制器。页面控制器是为那些每个界面都是其他界面的一份子的app设计的。

`不要使用页面控制器显示在层级中或其他复杂结构中的视图。`页面控制器不会显示一个视图是如何和其他视图关联的，而且它不会显示哪个视图属于哪个点，所以它不能帮助用户导航到一个特定的视图。

`不要显示太多的点。`超过10个点会让用户很难一瞥就数清楚，超过20个打开的视图会让按顺序查看变得很费时。如果用户可以在你的app中打开超过20个视图，考虑在不同的结构中显示这些视图，提供关于这些视图更多的信息并且允许非顺序的导航。

`在打开的视图的地步边缘和屏幕的底部边缘中间垂直居中页面控制器。`在这个位置，页面控制器总是可以不阻碍用户看见。
...
![](https://github.com/Cloudox/iOS-Human-Interface-Guidelines/blob/master/UI%20Elements/Temporary%20Views/3.jpeg)
...
![](https://github.com/Cloudox/iOS-Human-Interface-Guidelines/blob/master/UI%20Elements/Temporary%20Views/4.jpeg)
...
`NOTE`  
如果你需要给人们提供两个以上的选项，考虑使用动作表单来代替（查看[Action Sheet](https://developer.apple.com/library/ios/documentation/UserExperience/Conceptual/MobileHIG/Alerts.html#//apple_ref/doc/uid/TP40006556-CH14-SW36)来学习如何使用动作表单）。
...
更多内容参见[我的博客](http://blog.csdn.net/column/details/cloudox-column0.html)  
[回到目录](#Catalogue)
